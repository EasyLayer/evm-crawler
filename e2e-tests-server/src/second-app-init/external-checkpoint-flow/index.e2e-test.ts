...
describe('EVM Crawler: Second Initialization External Checkpoint Flow', () => {
  ...
  it('should rollback EventStore to external checkpoint when local write model is ahead', async () => {
    ...
    // No events with blockHeight > 1 remain after rollback
    expect(rows.some((r: any) => r.blockHeight > 1)).toBe(false);
    expect(rows.some((r: any) => r.type === 'EvmNetworkBlocksAddedEvent' && r.blockHeight === 2)).toBe(false);
    expect(rows.some((r: any) => r.type === 'EvmNetworkBlocksAddedEvent' && r.blockHeight === 3)).toBe(false);

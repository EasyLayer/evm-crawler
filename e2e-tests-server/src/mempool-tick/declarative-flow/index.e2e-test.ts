...
// Network is at -1: no blocks come through, mempool runtime is the only active path.
...
// subscribe-ws strategy: emit all known hashes synchronously into the callback when
// the loader subscribes. The flushTimer will then drain the buffer through
// getPendingTransactionByHash → snapshot → recordLoaded → load_tracker populated.
jest
...
describe('EVM Crawler: Mempool Tick — Declarative Flow', () => {
  ...
  it('emits at least one MempoolTickEvent (the user model saw the mempool source)', async () => {
    const ticks = await dbService.all(`SELECT type FROM "${AGGREGATE_ID}" WHERE type='MempoolTickEvent'`);
    expect(ticks.length).toBeGreaterThanOrEqual(1);
  });

  // it('emits a MempoolTxSeenEvent for each loaded mempool tx', async () => {

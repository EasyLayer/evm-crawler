...
export class GetContractSummaryQueryHandler implements IQueryHandler<GetContractSummaryQuery> {
  constructor(private readonly modelFactory: ModelFactoryService) {}
  public async execute(): Promise<any> {
    const model = await this.modelFactory.restoreByCtor(Erc20TransferMonitor);
    return model.getContractSummary();
  }
}
export class GetRecentTransfersQuery {}
@QueryHandler(GetRecentTransfersQuery)
export class GetRecentTransfersQueryHandler implements IQueryHandler<GetRecentTransfersQuery> {
  constructor(private readonly modelFactory: ModelFactoryService) {}
  public async execute(): Promise<any> {
    const model = await this.modelFactory.restoreByCtor(Erc20TransferMonitor);
    return model.getRecentTransfers();
  }
}

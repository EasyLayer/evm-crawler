...
export class NativeBalanceWatcher extends Model {
  ...
  public async processBlock(ctx: ProcessBlockExecutionContext): Promise<void> {
    ...
    this.applyEvent('NativeBalanceSnapshotCaptured', ctx.block.blockNumber, {
  ...
  protected onNativeBalanceSnapshotCaptured(event: any): void {
    ...
    for (const item of payload.balances ?? []) {
      this.balances.set(normalizeAddress(item.address), item.balanceWei);
    }
  }
  ...
  public getAllBalances(): Record<string, string> {
    ...
    for (const address of this.wallets) {
      result[address] = this.balances.get(address) ?? '0';
    }
...
export async function loadBalancesAtBlock(
  ...
  if (!httpClient) {
    throw new Error('Active EVM HTTP provider is not available in processBlock context');
  }
...
function readWatchAddresses(): string[] {
  const raw = readEnvValue('WATCH_ADDRESSES') ?? DEFAULT_WATCH_ADDRESSES.join(',');
  const addresses = raw
    .split(',')
...
function readEnvValue(key: keyof BrowserEnvLike): string | undefined {
  const globalEnv = (globalThis as any)?.__ENV as BrowserEnvLike | undefined;
  if (globalEnv?.[key]) return globalEnv[key];
